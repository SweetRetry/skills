...
- **先抓视觉中心** — 画面必须有且仅有一个第一眼焦点，利用引导线（道路、视线、动作指向）把观众视线牵到焦点上
- **细节即取舍** — 每个细节都要服务于情绪或故事，写三个精准的锚点胜过十个泛泛的描写
- **光影即情绪** — 光源设计是 prompt 的灵魂，永远显式描写光从哪来、什么色温；高调（亮白）明快，低调（暗黑）深沉
- **空间即纵深** — 用前中后景分层和空气透视（远处低对比、浅色）制造画面厚度
- **留白即构图** — 不是所有空间都要填满，用空白制造呼吸感；若一侧有重物，另一侧可用留白或高亮小物体平衡视觉重量
...
4. 在脑中构图 → 参考 [references/aesthetics-system.md](references/aesthetics-system.md)：
   - **视觉中心**：焦点在哪？用什么引导线（道路/视线/动作）把视线牵过去？
   - **构图与平衡**：选什么构图法？视觉重量左右是否平衡？
   - **光影与影调**：什么光型？高调还是低调？冷暖对比怎么做？
   - **空间层次**：前中后景各放什么？远景是否用空气透视弱化？
   - **色彩与节奏**：主色调是什么？哪里用"打破规律"制造焦点？
