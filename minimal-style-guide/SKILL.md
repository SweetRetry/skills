...
| ID | 属性 | 规范值 | 理由 |
|----|------|--------|------|
| **M.H1** | Token 化 | 禁止硬编码 Hex，组件层使用语义 Token | 跨风格融合的基础设施，所有风格共享此约束 |
| **M.H2** | 色彩空间 | oklch | 感知均匀色彩空间，混合项目的标准；已在 CSS Variables 层定义 |
| **M.H3** | 单色主导 | 界面 90% 无彩色，全局仅 1 种强调色 (`--primary`) | 信噪比最大化的核心哲学——增加强调色将破坏极简特征 |
| **M.H4** | 动效位移 | ≤10px | 超过此阈值将失去"克制"特征，在感知上变为 Expressive 风格 |
| **M.H5** | 对比度 | WCAG AA (≥4.5:1) | 无障碍强制要求 |
| **M.H6** | OLED 安全暗色 | `oklch(0.145 0 0)` ≈ #1A1A1A，禁止纯黑 | OLED 拖影防护 + 暗色模式的视觉舒适度 |

### 软约束 (Soft Constraints — 可由 Director 覆盖)

| ID | 属性 | 默认值 | Director 可调整范围 | 触发条件 |
|----|------|--------|-------------------|----------|
| **M.S1** | 圆角 | `rounded-md` (6px) / `rounded-lg` (8px) | 可降至 `rounded-none` (0px) 或 `rounded-sm` (2px) | 当 Swiss 为项目主风格时，Minimal 区域可跟随直角以保持全局几何一致性 |
| **M.S2** | 留白率 | 桌面 40%+ | 可降至 25–30% | 当与 Swiss 融合且整体信息密度需求较高时 |
| **M.S3** | 动效时长 | <400ms | 可缩短至 <300ms | 当 Swiss 为项目主风格，需要统一到更快的功能性节奏时 |
| **M.S4** | 减法表面 | 默认 `bg-transparent border-none shadow-none` | 可使用 `bg-card border-border` | 当区域需要与 Swiss 边框容器视觉衔接时（如嵌入在 Swiss 布局中的内容区域）|
| **M.S5** | Bento 网格 | 4–6 列灵活布局 | 可对齐到 12 列 8pt 网格 | 当 Swiss 为项目中的任一风格，需要全局网格基线对齐时 |
| **M.S6** | Stagger 间隔 | 0.06s | 可缩短至 0.03–0.04s | 当 Swiss 为主风格，整体交互节奏需要更快时 |
